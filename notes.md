...
- **Procedural map**: islands placed with min-distance rejection inside an oval play area, seeded RNG (mulberry32). Two presets: small (4200×3300, 17 islands per layer) and large "Grand Voyage" (8400×6600, 68 per layer). Density was tuned down once rumors/trading started paying in chart knowledge — information is only worth what the sailing costs. Old values put the median nearest isle ~4.0s away at cruise speed; current values ~5.7s (median nearest-neighbor 618px, measured over 6 seeds × 3 layers). Saves persist their own world dims (schema v6), so existing voyages keep the geometry they were generated with
...
- **Air streams (weather)**: a few rivers of wind per layer, deterministic from (seed, layer) like the islands — regenerated on load, nothing saved. Inside a band the ship is pushed along the stream's tangent (capped at 65 px/s where streams overlap): sail with one and surge, fight one and crawl. Character per altitude: the High Reaches run four hard narrow gales, the Cloud Sea three steady trades, the Hollow Deeps two broad soft sea-breezes. Winds are sky, not terrain, so they draw *above* fog — drifting streaks on the map, dashed flow-lines with arrowheads on the chart — making them routing information you can plan deliveries around. The HUD shows how the local stream treats your heading (following +32 / against −18 / across 24). Wind feathers out within 140px of a target so arrivals stay clean, and a becalmed ship doesn't creep — no wind while parked
...
### Feel & polish
- Wind/current vectors that nudge the ship — done: see air streams above. Open refinements: gusts/variability over time, storms as hazard-weather, and whether headwind routes need a "tack" mechanic to stay fun
